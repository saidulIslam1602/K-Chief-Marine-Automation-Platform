using HMI.Platform.Tests.TestHelpers.Performance;
using Xunit;

namespace HMI.Platform.Tests.Examples.Performance;
...
public class VesselPerformanceTests : PerformanceTestBase
{
    ...
    public void CreateVessel_Should_Complete_Within_100ms()
    {
        AssertExecutionTime(() =>
        {
            ...
            var vessel = new HMI.Platform.Core.Models.Vessel
            {
                Id = "vessel-001",
                Name = "Test Vessel",
                Length = 100.0,
    ...
    public void ProcessVessels_Should_Handle_1000_Items_Within_5Seconds()
    {
        var vessels = Enumerable.Range(1, 1000)
            .Select(i => new HMI.Platform.Core.Models.Vessel
            {
                Id = $"vessel-{i:D3}",
                Name = $"Vessel {i}",
                Length = 100.0 + i
